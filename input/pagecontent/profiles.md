...
- **item.extension[clinicalAlert]**: Optional (0..1, Must-Support) boolean flag to mark a questionnaire item as clinically critical. Use cases include allergy questions (e.g., "Bekannte Allergien"), anticoagulant medication items, and endocarditis risk screening items. When set to `true`, PVS systems should surface the item prominently in the clinical workflow.
...
## FPDECoveragePrivat — Private / Self-Pay Coverage Routing
...
| Element | Cardinality | Extension | Description |
|---------|-------------|-----------|-------------|
| `status` | 0..1 | | Coverage status |
| `type` | 1..1 | | Must include `http://fhir.de/CodeSystem/versicherungsart-de-basis` with code `PKV` or `SEL` |
| `beneficiary` | 1..1 | | Reference(Patient) — the covered patient |
| `subscriber` | 0..1 | | Optional subscriber for classic PKV relationships |
| `payor` | 1..* | | Insurance carrier or direct payer, depending on the coverage type |
| `extension[billingAssignment]` | 0..1 | `billing-assignment` | Reference(Organization) — PVS / billing service buying receivables from this coverage relationship |

### Routing Pattern

1. The selected `Coverage` is the source of truth for external billing routing.
2. If `extension[billing-assignment]` is present, the referenced `Organization` is the PVS / billing service for that coverage.
3. Downstream claim generation maps that organization to `Claim.payee.type = other` and `Claim.payee.party`.
4. If the extension is absent, direct patient/self-pay remains valid and no external payee needs to be derived.

### Use Cases

1. **PKV with factoring:** Private insurance coverage routes receivables to a PVS via `billing-assignment`.
2. **Self-pay without PVS:** Coverage validates without the extension when the patient pays directly.
3. **Mixed coverage stack:** The same patient can keep GKV coverage without routing extensions and a separate private/self-pay coverage with PVS routing.

### PVS Implementation Note

When recording private billing coverage in a PVS:
1. Create a `Coverage` resource based on `FPDECoveragePrivat`
2. Populate `type` with `PKV` or `SEL` from `VersicherungsartDeBasis`
3. Set `beneficiary` to the covered patient and `payor` to the insurer or direct payer
4. Add `billing-assignment` only when receivables are assigned to an external billing service
5. Capture the corresponding patient consent as a separate `Consent` resource scoped to the same `Coverage`
6. Validate the instance against `FPDECoveragePrivat` before submission

See examples: `ExampleCoveragePrivat`, `ExamplePvsOrganization`, `ExampleConsentBillingAssignment`.
