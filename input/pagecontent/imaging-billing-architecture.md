...
This page describes the 3-layer architecture for imaging billing in the German practice management FHIR IG. It covers the role of external FHIR ConceptMaps, the ChargeItemDefinition catalog, the PLATFORM rule engine, and the R4 Subscription pattern used for workflow event triggers.
...
| **Business Logic** | PLATFORM Rule Engine | External service | Validation, budget limits, audit, final billing decision |
...
PVS adapters and PLATFORM use `ConceptMap/$translate` to look up initial billing code candidates. These suggestions feed into the ChargeItemDefinition catalog lookup and the PLATFORM rule engine for final validation.

**Important**: A `$translate` match is a recommendation, not a billing decision. The PLATFORM rule engine always makes the final billing decision after applying budget constraints (RLV/QZV), patient context, and Kassenspezifische rules.
...
### Layer 3 — Business Logic (PLATFORM Rule Engine)

PLATFORM is an external rule engine. It:
...
PLATFORM rule definitions live outside this IG. See [Plan-Library vs. Rule-Execution Boundary](plan-library-boundary.html) for the general boundary rationale.
...
PLATFORM instantiates these templates at runtime, replacing the placeholder `channel.endpoint` with the real service URL.
