...
//   value directly; the PLATFORM subscription handler checks the report-substatus
...
Description: "R4 Subscription template. Triggers when a DiagnosticReport reaches status=final (required for report-substatus#signed per IG). PLATFORM post-filters by the report-substatus extension (value=signed) to confirm the trigger. Used to hand off signed studies to the workflow engine."
Usage: #example
* status = #requested
...
// Use status=final as the broad trigger; PLATFORM handler applies the substatus filter post-receive.
* criteria = "DiagnosticReport?status=final"
* channel.type = #rest-hook
