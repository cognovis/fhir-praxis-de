// INTENTIONAL SYNTAX ERROR FOR RED PHASE — missing ConceptMap keyword
Id: scheinart-to-kvdt
Title: "ScheinartCS → KVDT Scheinuntergruppe"
Description: "Mappt PVS-interne Scheinart-Codes (ScheinartCS) auf KVDT-offizielle Scheinuntergruppe-Codes (KBV_CS_SFHIR_KBV_SCHEINART). Nur GKV-relevante Codes haben eine Entsprechung."
...
* group[0].element[0].target[0].relationship = #related-to
...
* group[0].element[1].target[0].relationship = #related-to
...
* group[0].element[2].target[0].relationship = #related-to
* group[0].element[2].target[0].comment = "Notfallschein"
* group[1].source = "https://fhir.cognovis.de/praxis/CodeSystem/scheinart"
* group[1].target = "https://fhir.kbv.de/CodeSystem/KBV_CS_SFHIR_KBV_SCHEINART"
* group[1].unmapped.mode = #fixed
* group[1].unmapped.code = #pkv
* group[1].unmapped.display = "Kein KVDT-Mapping (PKV/BG/IGeL nicht in KBV Schlüsseltabelle)"
