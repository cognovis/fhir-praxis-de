...
**Companion ADR**: `adapter/docs/adr/ADR-027-privacy-and-compliance.md`
...
artefact that adapter codegen turns into `pii-fields.ts`, eliminating today's
...
`llm-anonymized`). Primary consumer: adapter SDK + LLM-Gateway. Secondary:
...
Adapter maintains hand-curated `pii-fields.ts` at multiple sites:
...
Dental agents go live in 1–2 weeks. The Pvs-PVS data path covers patient,
...
implementation lives in adapter (codegen + AnonymizingTransport +
LLM-Gateway tokenization). See the adapter workstream `adr-027-privacy`
...
5. Adapter-1bx codegen runs (in adapter-CI or via `bun update`), `pii-fields.ts` regenerates
6. Adapter builds; new fields are now redacted / scrubbed / generalized per mode
**Postcondition**: New resource type covered without adapter-side hand-edits
**Exceptions**: CI rejection if (a) negative-test-corpus matches scrub patterns, (b) version-pin in adapter stale, (c) coverage regression vs prior version

### UC-2: Adapter consumes IG at build time
**Actor**: adapter build pipeline (CI or local `bun run codegen`)
**Precondition**: New IG version published; adapter pinned to that version in `package.json`
...
**Postcondition**: Adapter consumers (`AnonymizingTransport`, LLM-Gateway tokenization layer, PVS Faker pipeline) read from regenerated files
...
**Actor**: adapter LLM-Gateway processing an `ExternalAnonymizedPrompt` per ADR-027 §6
...
| `legacyCompat` | boolean | No | True if this entry exists to preserve coverage of legacy hardcoded adapter PII lists (Round 8 Q4) |
...
- **Ephemeral / non-IG**: Per-request mapping (LLM-Gateway, ADR-027 §6) — lives in `@adapter/llm-gateway` runtime memory, NEVER in the IG
...
**FR-031**: The IG build MUST produce a TypeScript-importable snapshot for adapter codegen consumption (file path TBD with adapter-1bx — see AW-1).
- **Priority**: Should
- **Acceptance**: adapter-1bx codegen successfully consumes the artefact; integration test in adapter CI.
...
**FR-042**: CI MUST verify reverse-coverage against the legacy adapter hardcoded list (per Round 8 Q4).
- **Priority**: Must
...
### Preserved Behavior (Brownfield → adapter migration)

**FR-P-001**: The IG MUST cover every field currently in adapter's hand-maintained `PII_FIELDS = { Patient: ['name', 'identifier', 'address', 'telecom', 'photo'], Practitioner: ['name', 'telecom', 'address', 'photo'], RelatedPerson: ['name', 'telecom', 'address'] }` for mode `agent-readonly`. (See FR-042 reverse-coverage gate.)

**FR-P-002**: The IG MUST cover every field currently in adapter's hand-maintained `FREE_TEXT_FIELDS = { DocumentReference: ['description'], AllergyIntolerance: ['note[].text'] }` for mode `agent-readonly`.

**FR-P-003**: The IG free-text scrub patterns MUST preserve the adapter intentional false-positive trade-off: titled-names only (`Dr.`, `Hr.`, `Fr.` + name), NOT bare `Capitalised Capitalised` bigrams. Negative test corpus enforces this.

**FR-P-004**: The IG-declared algorithm for `cryptoHash` MUST produce bit-identical anon-IDs to the current adapter `hashId(salt, id)` function for the same `(salt, id)` input.
...
**NFR-030**: The IG MUST document the upgrade-loop SLA (Round 2 Q4 = (c)): Issue → IG-PR → Release → Adapter-Codegen-Run, target end-to-end latency ≤ 1 working day for security-relevant fixes.
...
**Consumers**: adapter codegen, Aidbox `$fhir-package-install`
...
**Scenario**: Adapter consumer encounters a resource type the IG does not list (e.g. zukünftiger FHIR R5 type, or DE-Custom-Resource added after IG was last bumped).
...
**Expected Behavior**: Per adapter current implementation: rewrite `fullUrl` based on the *original* `resource.id` BEFORE hashing the resource.id — order-sensitive.
...
**Expected Behavior**: Recursive reference rewriting per adapter current behavior (see ADR-027 §5).
...
- `@adapter/fhir-de` codegen (adapter-1bx) — primary
- `@adapter/llm-gateway` tokenization layer (adapter-9qg6 et al.) — primary
- `@adapter/adapter-common` PVS Faker pipeline (adapter-oah) — secondary (field list only; Faker generators stay outside IG)
- Aidbox server-side `$de-identify` (future, optional)

### Migration

- adapter currently runs hand-maintained `pii-fields.ts`; codegen migration in adapter-1bx replaces it. Reverse-coverage CI gate (FR-042) prevents regression.
...
- Major bump triggers adapter semver-major (downstream)
...
**v0.x.x patches**: bug fixes, missing-field additions, scrub-pattern refinements that surface during adapter-1bx and adapter-9qg6 consumption. Each adds a coverage row, never removes (NFR-031).

**v1.0.0 trigger** (status flips from `draft` to `active`):
- adapter-1bx codegen has consumed the IG cleanly for ≥ 30 days
- adapter-9qg6 LLM-Gateway tokenization has consumed the IG cleanly for ≥ 30 days  
...
**BC-3**: When two consumers (e.g. adapter and Aidbox) process the same input resource with the same salt and the same IG version, they produce bit-identical `cryptoHash` output.
...
**BC-7**: When the IG's coverage of legacy adapter hardcoded fields drops vs the prior version, CI MUST fail.
...
**NB-6**: The IG MUST NOT specify a salt-rotation policy. Salt lifecycle is operational and lives in the adapter-5bdl runbook. The IG only states the salt requirements (≥256 bit entropy, fail-closed). Reason: per Round 4 Q1 + ADR-027 §5.
...
**NB-8**: The IG MUST NOT contain real PII in `input/examples/`. Examples MUST be synthetic, generated via deterministic Faker seeds (consistent with `adapter/packages/install-pvs/` and `install-dental-pvs/` patterns), and each example file MUST carry a `meta.tag` referencing the data-provenance CodeSystem (FR-070):
...
- **Data Flow Out**: `package.tgz` download by adapter codegen, by Aidbox `$fhir-package-install`
- **Failure Mode**: registry unreachable at consume-time → adapter CI build fails (vendored fallback is the runbook escape per fhir-praxis-de's `vendor/` pattern, FR-051)
- **Timeout Strategy**: adapter codegen: 60s download timeout; vendored fallback if registry returns 5xx
...
### Adapter codegen (`@adapter/fhir-de`)

- **Data Flow In**: this IG's manifest sidecar
- **Data Flow Out**: regenerated `pii-fields.ts`, `free-text-patterns.ts`, `quasi-id-generalizers.ts` in adapter repo
- **Failure Mode**: schema mismatch → adapter build fails (consumer-side); IG repository's CI is unaffected (consumer responsibility)
- **Timeout Strategy**: codegen runs synchronously in adapter CI
...
**AW-1 (RESOLVED)**: Adapter-codegen artefact format = **both**, with explicit roles.
- Canonical source: `Library` FHIR resource (FHIR-native, validatable, Aidbox-loadable)
- Build-time-derived: JSON sidecar at `output/manifest/pii-fields.json` (simple format optimized for adapter codegen — no FHIR machinery required in adapter build step)
- The build script extracts the JSON from the Library's `content[0].data`. Adapter-1bx reads the JSON sidecar; Aidbox `$fhir-package-install` consumes the Library. See FR-030 + FR-031 for acceptance.
...
- **Codes**:
  - `synthetic-faker-generated` — programmatically generated via `@faker-js/faker` with deterministic seed (matches adapter install-pvs/install-dental-pvs convention)
...
- **OQ-3 (RESOLVED 2026-05-02)**: Cross-repo bead-coordination handled via documentation in fpde-7yo notes; wave-orchestrator dispatches IG repo first, adapter workstream (`bd list --label=adr-027-privacy`) consumes the published artefact.

- **OQ-4 (RESOLVED 2026-05-02)**: `status: draft` for v0.1.0 through v0.x.x; flips to `status: active` at v1.0.0 once the adapter consumption gate (≥30 days clean consumption by adapter-1bx + adapter-9qg6) is satisfied. See updated Section 10.
...
- **OQ-R-2** (Risk): Adapter-1bx might find that the IG's manifest schema doesn't match what their codegen wants to consume. — **Mitigation**: AW-1 resolved before IG v0.1.0; cross-team review during sub-bead 1 (Format-Entscheidung).
